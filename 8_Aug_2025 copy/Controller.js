...
class Brainy {
    ...
    createDiscus(){
        ...
        console.log("Data dari localStorage (discus):", listDiscus)

        // posts.innerHTML = ''

        listDiscus.forEach((discus, index) => {
            console.log(discus.message)
            const user = from.find(u => u.id === parseInt(discus.userId))
            ...
            <div class="flex flex-col items-start m-3 gap-4 p-4 bg-gray-50 rounded-lg">
                ...
                <div class=" flex flex-col gap-3 items-end">
                    <p class="bg-gray-200 rounded-sm w-[1120px] p-2">${discus.message}</p>
                    <div class=" flex">
                        ...
                        <div class=" bg-gray-100 p-2 border-gray-300 border-2 rounded-lg">
                            <h5 class="mb-2 block font-sans text-l font-semibold leading-snug tracking-normal text-blue-gray-900 antialiased">
                                from: ${user.name}
                            </h5>
                            <p class="bg-gray-200 rounded-sm w-[1090px] p-2">${discus.message}</p>
                        </div>
                    </div>
                </div>
                <div class="center relative inline-block select-none whitespace-nowrap rounded-lg bg-blue-500 py-2 px-3.5 align-baseline font-sans text-xs font-bold uppercase leading-none text-white">
                    <div class="mt-px">${discus.course}</div>
                </div>
                <div>
                    <button id="" onclick="replyTag(${discus.id})" class="bg-gray-200 p-2 rounded-lg">Jawab</button>
                    <button onclick="deleteQuest(${discus.id})" class="bg-red-400 p-2 rounded-lg">Delete</button>
                </div>
            </div>`
        })
    }
...
// deleteQuest(${quest.id})
    ...
    replyTag(idQuest){
        const tagData = JSON.parse(localStorage.getItem("quest") && localStorage.getItem("discus"))
...
function deleteQuest(idQuest){
    ...
    let isQuest = JSON.parse(localStorage.getItem("quest"))
    let isDiscus = JSON.parse(localStorage.getItem("quest"))

    let delQuest = isQuest.filter(q => q.id !== idQuest)
    let delReply = isDiscus.filter(r => r.id !== idQuest)
    if(delQuest){
        console.log("menghapus data", delQuest)
        // localStorage.setItem("quest", JSON.stringify(delQuest))
    }else{
        console.log("menghapus reply", delReply)
    }
